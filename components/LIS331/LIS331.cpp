...
void LIS331::setI2CHandler(i2c_master_dev_handle_t dev_handle)
{
    this->I2Chandler = dev_handle;
}
...
uint8_t LIS331::getREG(uint8_t address)
{
  uint8_t data = 0;
  LIS331::LIS331_read(address, &data, 1);
  return data;
}
...
void LIS331::LIS331_write(uint8_t reg_address, uint8_t *data, uint8_t len)
{
  if (mode == USE_I2C)
  {
    // ESP I2C write handling code
    uint8_t write_buffer[2];

    write_buffer[0] = reg_address;
    std::memcpy(&(write_buffer[1]), data, len);
    
    i2c_master_transmit(I2Chandler, write_buffer, len+1,-1);
...
void LIS331::LIS331_read(uint8_t reg_address, uint8_t *data, uint8_t len)
{
  if (mode == USE_I2C)
  {
    // ESP I2C read handling code

    uint8_t read_buffer[1];

    read_buffer[0] = reg_address;

    i2c_master_transmit_receive(I2Chandler, read_buffer, 1, data, len, -1);
